/** Stable Unsplash fallbacks when API images fail or are missing. */
 ...
 * @returns {string[]}
 */
export function resolveProductImages(images, seed = '') {
 ...
 * @param {string} [seed]
 */
export function resolveProductImage(src, seed = '') {
  if (isUsableUrl(src)) return src.trim()
  return resolveProductImages([], seed)[0]
}
