...
export function AdminPageHeader({ eyebrow, title, subtitle, actions }) {
  return (
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        {eyebrow ? (
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-indigo-300/90">{eyebrow}</p>
        ) : null}
...
export function AdminStatCard({ label, value, hint }) {
  return (
    <motion.div
      layout
      className="rounded-tn-2xl border border-white/10 bg-white/5 p-5 shadow-inner backdrop-blur-md transition hover:border-indigo-400/30 hover:bg-white/[0.07]"
      whileHover={{ y: -2 }}
